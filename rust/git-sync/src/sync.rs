...
async fn crud_create(
    ...
    match sync.db() {
        Db::Sqlite(pool) => {
            ...
            db::tx::sqlite::ensure_file_row(&mut tx, sync.worktree_id(), file_path).await?;
            let lookup =
                db::tx::sqlite::lookup_commits(&mut tx, sync.worktree_id(), file_path, path, key)
                    .await?;
            ...
            .await?;
            format_owner =
                db::tx::sqlite::file_format(&mut tx, sync.worktree_id(), file_path).await?;
            db::tx::sqlite::replace_aliases(
                &mut tx,
                id,
                &compute_aliases(
                    sync,
        ...
        Db::Postgres(pool) => {
            let mut tx = pool.begin().await?;
            db::tx::pg::ensure_file_row(&mut tx, sync.worktree_id(), file_path).await?;
            let lookup =
                db::tx::pg::lookup_commits(&mut tx, sync.worktree_id(), file_path, path, key)
                    .await?;
            if lookup.record_id.is_some() {
                return Err(Error::AlreadyExists {
                    file_path: file_path.to_string(),
            ...
            .await?;
            format_owner = db::tx::pg::file_format(&mut tx, sync.worktree_id(), file_path).await?;
            db::tx::pg::replace_aliases(
                &mut tx,
                id,
                &compute_aliases(
                    sync,
...
async fn crud_update(
    ...
    match sync.db() {
        Db::Sqlite(pool) => {
            ...
            db::tx::sqlite::update_record(&mut tx, id, &json_text).await?;
            let format_owner =
                db::tx::sqlite::file_format(&mut tx, sync.worktree_id(), file_path).await?;
            db::tx::sqlite::replace_aliases(
                &mut tx,
        ...
        Db::Postgres(pool) => {
            ...
            db::tx::pg::update_record(&mut tx, id, &json_text).await?;
            let format_owner =
                db::tx::pg::file_format(&mut tx, sync.worktree_id(), file_path).await?;
            db::tx::pg::replace_aliases(
                &mut tx,
                id,
                &compute_aliases(
...
async fn crud_delete(
    ...
    match sync.db() {
        Db::Sqlite(pool) => {
            ...
            db::tx::sqlite::delete_record(&mut tx, id).await?;
            tx.commit().await?;
        }
        #[cfg(feature = "postgres")]
        Db::Postgres(pool) => {
            ...
            db::tx::pg::delete_record(&mut tx, id).await?;
            tx.commit().await?;
        }
    }
    Ok(())
