...
# if value is list
def recursiveLookupPath(value, keys, context=None):
  """
  Returns a list of final values or None if path didn't match
"""
  if not keys:
    return [value]
  ...
  while isinstance(keys[0], list):
    #it's a filter
    #print ('filter', keys, value)
    filter = keys.pop(0)
    if filter[0] == '!':
      if recursiveLookupPath(value, filter[1:], context) is not None:
        return None
    elif recursiveLookupPath(value, filter, context) is None:
      return None
    #print ('match', keys, value)
    if not keys:
      return [value]
  ...
  if isinstance(value, list):
    try:
      key = int(keys[0])
      #print('index', keys, key, value)
      keys.pop(0)
    except ValueError:
      # key is not a list index
      firstMatch = keys[0] == '?'
      if firstMatch:
        keys.pop(0)
        if not keys:
      ...
      def makeList():
        for item in value:
          #print('keys', keys, 'item', item)
          nextvalue = recursiveLookupPath(item, keys, context)
          if nextvalue is not None:
            if firstMatch:
              yield item
              break
            #nextvalue is always a list, flatten it
            for nextitem in nextvalue:
              yield nextitem

      value = list(makeList())
      #print('firstMatch', firstMatch, 'keys', keys, 'v', value)
      return value if value else None
  else:
    key = keys.pop(0)
    ...
    if callable(key):
      ...
      try:
        ...
        if not comparor(value, compare):
          return None
      except:
        if comparor is not operator.ne:
          return None
    # if key == '*':
    #   if isinstance(value, dict):
  ...
  try:
    ...
    if Ref.isRef(value):
      ...
      if not value:
        ...
        value = value[0] #XXX why?

    return recursiveLookupPath(value, keys, context)
