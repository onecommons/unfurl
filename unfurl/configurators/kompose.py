        ...
        operations:
          configure:
            ...
            inputs:
              ...
              livenessProbe: null # disable or replace default livenessProbe
...
class KomposeConfigurator(ShellConfigurator):
    ...
    def run(self, task):
        ...
        if "livenessProbe" in task.inputs:
            livenessProbe = task.inputs["livenessProbe"]
        else:
            livenessProbe = default_livenessProbe
        ...
        jobRequest, errors = task.update_instances([
            _load_resource_file(task, out_path, filename, livenessProbe) for filename in files
        ])
        if errors:
...
def set_livenessProbe(resource, livenessProbe):
    obj = resource
    for key in "spec/template/spec/containers".split("/"):
        obj = obj.get(key)
        if not obj:
            break
    if key == "containers" and obj and "livenessProbe" not in obj[0]:
        if callable(livenessProbe):
            livenessProbe = livenessProbe(obj[0]["ports"][0]["containerPort"])
        obj[0]["livenessProbe"] = livenessProbe
        return True
    return False
...
def _load_resource_file(task, out_path, filename, livenessProbe):
    with open(Path(out_path) / filename) as f:
        definition = f.read()
    definition = yaml.load(definition)
    if livenessProbe:
        set_livenessProbe(definition, livenessProbe)
