# Generated by tosca.yaml2python from unfurl/configurators/templates/helm.yaml at 2025-05-07T16:10:44 overwrite not modified (change to "overwrite ok" to allow)
...
"""
    ...
    def configure(self, **kw: Any) -> Any:
        return self.execute(
            helmcmd=Eval(
                (
                    '{{ "upgrade" if ".::.present" | eval else "install"}} {%if SELF.version | '
                    'default("") %}--version {{SELF.version}}{% endif %}'
                )
            ),
        )
