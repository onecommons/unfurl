...
# start job: move old "planned" folders
# during job:
...
def move_dirs(root_dir, subdir):
    ...
    if os.path.exists(subdir_path):
        if len(subdir.parts) > 1:
            ...
            rmtree(subdir_path)
    assert subdir.parts
    subdir_base = subdir.parts[0]
    if os.path.isdir(root_dir):
        for de in os.scandir(root_dir):
            if de.is_dir() and de.name != subdir_base:
                rename_dir(de.path, os.path.join(subdir_path, de.name))
