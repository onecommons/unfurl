...
def _get_file_module_spec(fullname, origin_path, modules):
    ...
    return spec_from_loader(fullname, loader, origin=origin_path)


def _get_dir_module_spec(fullname, repo_path, modules):
...
class RepositoryFinder(PathFinder):
    ...
    @classmethod
    def find_spec(
        cls, fullname: str, path=None, target=None, modules=None
    ) -> Optional[ModuleSpec]:
            ...
            if repo_path:
                if len(names) == 2:
                    if os.path.exists(repo_path):
                        return _get_dir_module_spec(fullname, repo_path, modules)
                    else:
                        logger.error(
                    ...
                    if os.path.isdir(origin_path) and not os.path.isfile(
                        ...
                        return _get_dir_module_spec(fullname, origin_path, modules)
                    return _get_file_module_spec(fullname, origin_path, modules)
            else:
                logger.error(
        ...
        # XXX special case service-template.yaml as service_template ?
            ...
            if len(names) == 1:
                return _get_dir_module_spec(fullname, dir_path, modules)
            else:
                if SKIP_LOADER:
                    return None
