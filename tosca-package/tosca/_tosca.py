...
class WritePolicy(Enum):
    ...
    def can_overwrite(self, input_path, output_path):
        if self == WritePolicy.always:
            return True
        if self == WritePolicy.never:
            return not os.path.exists(output_path)
        elif self == WritePolicy.older:
            return is_newer_than(output_path, input_path)
        else:  # auto
...
def is_newer_than(output_path, input_path):
    ...
    if (
        ...
        and os.stat(output_path).st_mtime_ns > os.stat(input_path).st_mtime_ns
    ):
        return True
    return False
